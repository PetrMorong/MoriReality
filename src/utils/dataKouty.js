import React from 'react'

export const data = {
  status: "Předprodej",
  title: "Wellness hotel Soutok Kouty",
  locationOne: "Rejhotice 131",
  locationTwo: "Loučná nad Desnou",
  cover: "vilaRepublikaCover",
  //   videoUrl: "https://youtu.be/vnrog-OczGQ",
  desc: (
    <>
     Čtyřhvězdičkový hotel s bistrem a prémiovým wellness v srdci Jeseníků.
Pouhých 800 metrů od skiareálu Kouty s 3. nejdelší sjezdovkou v ČR a lanovkou na Praděd.

V projektu vznikne 23 designových jednotek, všechny jsou k prodeji —
od komfortních apartmánů až po investiční ubytovací jednotky se stabilním výnosem. Majitelům nabízíme také kompletní správu, provoz i marketing.

Součástí resortu bude:

Privátní wellness s prémiovým vybavením, stylové bistro, hotelové pokoje a apartmány ve čtyřhvězdičkovém standardu, profesionální správa a plný servis pro investory.

Investujte do hotelu v jedné z nejrychleji rostoucích lokalit Jeseníků.
Zajistíme vysokou obsazenost, bezstarostný provoz a dlouhodobý výnos.
    </>
  ),
  sliderImages: [
    {
      imageUrl:
        "v1763645408/Kouty/01-min_uxjr1h.jpg",
    },
    {
      imageUrl:
        "v1763645411/Kouty/04-min_ob6z5p.jpg",
    },
    {
      imageUrl:
        "v1763645409/Kouty/02-min_ez8vl9.jpg",
    },
    {
      imageUrl:
        "v1763645409/Kouty/03-min_mlwxdh.jpg",
    },
    {
      imageUrl:
        "v1763645405/Kouty/recepce_01_tyr3cq.png",
    },
    {
      imageUrl:
        "v1763645392/Kouty/1000015941_wzwq3z.png",
    },
    {
      imageUrl:
        "v1763645403/Kouty/1000015942_k415zz.png",
    },
    {
      imageUrl:
        "v1763645405/Kouty/1000015946_zr6ulp.png",
    },

     {
      imageUrl:
        "v1776933198/Kouty/rekoFront_xrooec.png",
    },
       {
      imageUrl:
        "v1776933211/Kouty/rekoBack_t3j9vo.png",
    },
       {
      imageUrl:
        "v1776934055/Kouty/rekobyt_azent6.png",
    },  {
      imageUrl:
        "v1776934219/Kouty/rekoBistro_n4fjvz.png",
    },
    
  ],
  hideArchitect: true,
  stats: [
    {
      value: "2000",
      highlighted: "m2",
      desc: "Plocha pozemku",
    },
    {
      value: "23",
      highlighted: "",
      desc: "Jednotek",
    },
    {
      value: "Až",
      highlighted: "2+kk",
      desc: "Dispozice",
    },
    {
      value: "Leden",
      highlighted: "2028",
      desc: "Termín Dokončení",
    },
  ],
  features: [
    {
      icon: "/features/sauna.png",
      desc: "Privátní wellness",
    },
    {
      icon: "/features/ski.png",
      desc: "Lyžárna",
    },
    {
      icon: "/features/table-football(1).png",
      desc: "Bistro",
    },
    {
      icon: "/features/view.png",
      desc: "4 hvězdičkový servis",
    },
    {
      icon: "/features/park.png",
      desc: "Uprostřed přírody",
    },
    {
      icon: "/features/parking-area.png",
      desc: "Parkovací místo",
    },
    {
      icon: "/features/smart-home.png",
      desc: "Chytrá domácnost",
    },
    {
      icon: "/features/door-lock.png",
      desc: "Self checkin",
    },
  ],

  sectionFourOneText: {
    title: "Originální koncept ",
    titleGold: "designových apartmánů",
    desc: "Naše apartmány sjednocuje promyšlený horský design, který vytváří harmonický a nadčasový celek. Každý prostor vychází z jednoho konceptu – přírodních materiálů, teplých tónů a důrazu na detail. Zároveň nabízíme možnost dokončení interiéru podle vlastních představ. Můžete si tak přizpůsobit finální podobu apartmánu přesně svému stylu – od materiálů až po designové prvky. Výsledkem je kombinace kvalitního základu a individuálního řešení, které dává každému apartmánu osobitý charakter.",
  },

  sectionFourTwoText: {
    title: "Wellness",
    titleGold: "s útulnou atmosférou",
    desc: "Pohodlí jsme podřídili vše. V přízemí domu vzniká designové wellness, kde můžete načerpat síly a užít si soukromí. K dispozici bude finská sauna s parním nárazem, odpočinková zóna, whirlpool vana pro čtyři osoby a zážitková sprcha s relaxačními programy. Rezervaci si jednoduše vytvoříte online a vstup do wellness probíhá pomocí kódu. Pro majitele apartmánů je vstup do wellness za zvýhodněnou, nákladovou cenu.",
  },

  sectionImageOne:
    "v1776926895/Kouty/01_KOUTY_pwndsy.jpg",
  sectionImageTwo:
    "v1763645392/Kouty/1000015941_wzwq3z.png",
  sectionCover:
    "v1776927320/Kouty/02_KOUTY_jqitds.jpg",

  ticksSection: [
    {
      highlighted: "Profesionální ",
      normal: "správa apartmánů ",
      desc: "Bez starostí, s jasným výnosem. O provoz apartmánu se postará tým Mori Reality – od prezentace, přes rezervace, úklid a údržbu až po vyúčtování. Nastavíme správu tak, aby dávala dlouhodobý smysl a majiteli přinášela stabilní výnos, nikoli starosti. Majitel může apartmán libovolně využívat pro sebe, nebo jej přenechat správě podle zvoleného modelu.",
    },
    {
      highlighted: "a vysoká obsazenost",
      normal: "Investiční model ",
      desc: "Atraktivní výnos díky silné poptávce po ubytování. Kouty i celé Jeseníky mají dlouhodobě vysokou obsazenost díky lyžování, cyklistice, turistice a lázeňským službám v okolí. Aparthotel navíc nabídne vlastní wellness, které výrazně podporuje obsazenost i mimo sezónu. Díky tomu může investor počítat se stabilním tokem rezervací a předvídatelným cashflow.",
    },
    {
      highlighted: "garance",
      normal: "Financování a  ",
      desc: "Pomůžeme vám úvěr nastavit tak, aby schválení dopadlo. Nákup ubytovací jednotky má specifika, která běžné banky často neumí správně posoudit. Proto zajišťujeme odbornou pomoc s financováním – od výběru vhodné banky až po přípravu podkladů. Při využití našeho smluvního partnera je navíc rezervace vratná, pokud banka úvěr neschválí.",
    },
  ],
 apartments: [
  {
    price: "Projekt v přípravě",
    // price: "2 590 000 Kč",
    number: "131/7",
    floor: "2NP",
    layout: "1 + kk",
    size: "15,93 m²",
    terasa: "-",
    parking: "1",
    reserved: false,
  },
  {
    price: "Projekt v přípravě",
    // price: "2 790 000 Kč",
    number: "131/8",
    floor: "2NP",
    layout: "1 + kk",
    size: "17,83 m²",
    terasa: "-",
    parking: "1",
    reserved: false,
  },
  {
    price: "Projekt v přípravě",
    // price: "2 790 000 Kč",
    number: "131/9",
    floor: "2NP",
    layout: "1 + kk",
    size: "17,86 m²",
    terasa: "-",
    parking: "1",
    reserved: false,
  },
  {
    price: "Projekt v přípravě",
    // price: "3 990 000 Kč",
    number: "131/10",
    floor: "2NP",
    layout: "1 + kk",
    size: "31,03 m²",
    terasa: "8,96 m²",
    parking: "1",
    reserved: false,
  },
  {
    price: "Projekt v přípravě",
    // price: "3 990 000 Kč",
    number: "131/11",
    floor: "2NP",
    layout: "1 + kk",
    size: "31,76 m²",
    terasa: "7,05 m²",
    parking: "1",
    reserved: false,
  },
  {
    price: "Projekt v přípravě",
    // price: "3 590 000 Kč",
    number: "131/12",
    floor: "2NP",
    layout: "1 + kk",
    size: "28,10 m²",
    terasa: "7,90 m²",
    parking: "1",
    reserved: false,
  },
  {
    price: "Projekt v přípravě",
    // price: "3 190 000 Kč",
    number: "131/13",
    floor: "2NP",
    layout: "1 + kk",
    size: "24,03 m²",
    terasa: "7,90 m²",
    parking: "1",
    reserved: false,
  },
  {
    price: "Projekt v přípravě",
    // price: "5 490 000 Kč",
    number: "131/14",
    floor: "2NP",
    layout: "2 + kk",
    size: "39,56 m²",
    terasa: "12,74 m²",
    parking: "1",
    reserved: false,
  },
  {
    price: "Projekt v přípravě",
    // price: "3 590 000 Kč",
    number: "131/15",
    floor: "2NP",
    layout: "1 + kk",
    size: "27,79 m²",
    terasa: "9,79 m²",
    parking: "1",
    reserved: false,
  },
  {
    price: "Projekt v přípravě",
    // price: "4 490 000 Kč",
    number: "131/16",
    floor: "2NP",
    layout: "2 + kk",
    size: "39,56 m²",
    terasa: "12,74 m²",
    parking: "1",
    reserved: false,
  },
],
  otherProjects: [
   {
      image: "v1763643022/Rapotin/DSC01361-HDR-min_muyeuj.jpg",
      name: "Wellness resort",
      location: "Červený dvůr, Rapotín",
      link: "/cervenydvur",
    },
    {
      image: "v1689504195/vilaRepublikaFoto/356584285_1096124314696983_2263942071942825600_n_xv0nkq.jpg",
      name: "Vila Republika",
      location: "Velké Losiny",
      link: "/vila-republika",
    },
    {
      image: "v1724488767/hrabenov/DSC03066-HDR-min_ekphad.jpg",
      name: "Rezidence Hrabenov",
      location: "Ruda Nad Moravou",
      link: "/rezidence-hrabenov",
    },
  ],
  timelineFill: "30%",
  timeline: [
    {
      title: "Projektová dokumentace",
      date: "Listopad 2025",
    },
    {
      title: "Zahájení rekonstrukce",
      date: "Leden 2026",
    },
    {
      title: "Dokončení rekonstrukce fasády",
      date: "Říjen 2027",
    },
    {
      title: "Předání nemovitosti",
      date: "Leden 2028",
    },
  ],
  architectName: "Arch. Studio Reaktor",
  architectWord:
    "“Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium.“",
};